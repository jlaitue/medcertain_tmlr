...
import pandas as pd
import warnings
from utils.constants import EPSILON, LABEL_TITLES
...
@jax.jit
def upscaled_mimic_ce_loss_sigmoid(preds, Y): #shape (batch_size, labels)
    '''Cross-Entropy loss of multilabel/binary distribution for MIMIC using upscaled loss 
    on positive label-patient pairs
    ...
    '''
    label_prevalence = np.array([ 32.32, 7.99, 8.57, 37.36, 23.34, 16.40, 21.75, 
                                10.79, 31.03, 31.00, 11.82, 20.58, 41.05, 44.14, 
                                45.48, 7.08, 21.17, 16.12, 12.93, 6.29, 9.93, 
                                18.84, 28.37, 22.21, 18.01])

    pos_weight_factors = abs(label_prevalence - 100)/label_prevalence

    weight_matrix = pos_weight_factors*Y + (1-Y) # Ensures the losses of negative labels are kept
    # weight_matrix[weight_matrix==0] = 1 # Ensures the losses of negative labels are kept

    label_patient_loss = Y*jnp.log(preds + 1e-10) + (1 - Y)*jnp.log(1 - preds + 1e-10)

    upscaled_loss = label_patient_loss*weight_matrix
    return -jnp.sum(upscaled_loss, axis=0)
...
def selective_prediction_mimic(targets, preds, eval_metric):  # Y is of shape (n, num_labels)
    ...
    for label in range(num_labels):
        if num_labels == 1:
            print(f"     Processing label {label+1}")
        else:
