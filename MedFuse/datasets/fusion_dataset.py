...
class MIMIC_CXR_EHR(Dataset):
    def __init__(self, args, metadata_with_labels, ehr_ds, cxr_ds, split="train"):
        ...
        # This is used for UNI-CXR.
        # In the original MedFuse code, this Class is used to manage loading
        # all the uni-modal and fusion datasets
    ...
    def __getitem__(self, index):
        if self.args["data_pairs"] == "paired_ehr_cxr":
            ...
            ehr_data, labels_ehr = np.zeros((1, 76)), np.zeros(self.args["num_classes"])
...
def loadmetadata(args):
    # This was validated, files correct.
    cxr_metadata = pd.read_csv(f"{args['cxr_data_dir']}/mimic-cxr-2.0.0-metadata.csv") # We could load args['metadata_file']
    ...
    # This only applies to mimic-cxr-pheno.csv labels file,
    # if using mimic-cxr-2.0.0-chexpert.csv this filtering does not occur
    if args["labels_file"] == "mimic-cxr-pheno.csv":
        labels = pd.read_csv(f"{args['cxr_data_dir']}/{args['labels_file']}") 
        unique_cxr_studies = labels.study_id.unique()

        # Excluding patients that do not exist in the labels file for CLINICAL CONDITION task (ancient PHENOTYPING task)
        # print(len(cxr_merged_icustays_during))
        cxr_merged_icustays_during = cxr_merged_icustays_during.loc[cxr_merged_icustays_during.study_id.apply(lambda x: x in unique_cxr_studies)]
        # print(len(cxr_merged_icustays_during))
...
def load_cxr_ehr(args, ehr_train_ds, ehr_val_ds, cxr_train_ds, cxr_val_ds, ehr_test_ds, cxr_test_ds):
    print("LOADING MIMIC-FUSION CXR-EHR DATASET")
    ...
    if args["label_file_splits"] != "original": # Any of: original | seed_test | same_test | medfuse_test
        ...
        splits_labels_TRAIN_path = f"{args['ehr_data_dir']}/{args['task']}/train_listfile.csv"
        splits_labels_VAL_path = f"{args['ehr_data_dir']}/{args['task']}/val_listfile.csv"
        splits_labels_TEST_path = f"{args['ehr_data_dir']}/{args['task']}/test_listfile.csv"
    
    print("Train labels file: ", splits_labels_TRAIN_path)
    print("Val labels file: ", splits_labels_VAL_path)
    print("Test labels file: ", splits_labels_TEST_path)

    splits_labels_train = pd.read_csv(splits_labels_TRAIN_path)
    splits_labels_val = pd.read_csv(splits_labels_VAL_path)
    splits_labels_test = pd.read_csv(splits_labels_TEST_path)
    ...
    # The split flag is only used for the ratio in the init function
    # which is used only for self.args["data_pairs"] == "partial_ehr_cxr":
    train_ds = MIMIC_CXR_EHR(args, train_meta_with_labels, ehr_train_ds, cxr_train_ds, split="train") 
    val_ds = MIMIC_CXR_EHR(args, val_meta_with_labels, ehr_val_ds, cxr_val_ds, split="validate")
    test_ds = MIMIC_CXR_EHR(args, test_meta_with_labels, ehr_test_ds, cxr_test_ds, split="test")

    # print("META WITH LABELS")
    # print("\nTRAIN")
...
def printPrevalence(merged_file, args):
    if args["labels_set"] == 'phenotyping':
...
def my_collate(batch):
    ...
    if targets_ehr.shape[-1] != 25: # MORTALITY task
        ...
        # In MORTALITY we use the radiology labels for CXR
    ...
    # Created a 5-element-tuple within a list so that the rest of the code works, 
