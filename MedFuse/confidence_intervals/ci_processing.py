...
def ece(targets, preds):
    ...
    one_hot_preds = (preds > 0.5).astype(float) 
    one_hot_preds = jax.nn.one_hot(one_hot_preds, num_classes=2)
    one_hot_preds = (one_hot_preds.T * preds).T
    
    # Targets
    # one-hote encoded targets --> [[1,0], [1, 0], [0, 1]]
    one_hot_targets = jax.nn.one_hot(targets, num_classes=2)
    
    ece =  calibration(one_hot_targets, one_hot_preds)
    ece_label, ece_label_max = tuple([x*100 for x in ece])
...
def multilabel_bce_with_sigmoid(targets, preds):
    ...
    loss = -np.sum(targets*np.log(preds + 1e-10) + (1 - targets)*np.log(1 - preds + 1e-10))
...
def evaluate_new(df, selected_metric):
    ...
    if selected_metric == "AUROC":
        metric_value = roc_auc_score(df['y_truth'], df['y_pred'])

    elif selected_metric == "AUPRC":
        metric_value = average_precision_score(df['y_truth'], df['y_pred'])

    elif selected_metric == "SEL_AUROC_1":
       _,  metric_value = selective_prediction_eval_protocol_1(targets=df['y_truth'], preds=df['y_pred'], eval_metric="AUROC")
    
    elif selected_metric == "SEL_AUPRC_1":
        _, metric_value = selective_prediction_eval_protocol_1(targets=df['y_truth'], preds=df['y_pred'], eval_metric="AUPRC")

    elif selected_metric == "SEL_AUROC_3":
        _, metric_value = selective_prediction_eval_protocol_3(targets=df['y_truth'], preds=df['y_pred'], eval_metric="AUROC")
    
    elif selected_metric == "SEL_AUPRC_3":
        _, metric_value = selective_prediction_eval_protocol_3(targets=df['y_truth'], preds=df['y_pred'], eval_metric="AUPRC")

    elif selected_metric == "ECE":
        metric_value = ece(targets=df['y_truth'], preds=df['y_pred'])
    
    elif selected_metric == "NLL":
        metric_value = multilabel_bce_with_sigmoid(targets=df['y_truth'], preds=df['y_pred'])
...
def bootstraping_eval(df, selected_metric, num_iter):
    ...
    for _ in range(num_iter):
        ...
        if "SEL" in selected_metric or selected_metric == "NLL":
            y_truth = df["y_truth"]
            y_pred = df["y_pred"]
...
def get_model_performance(df, selected_metric):
    ...
    if "SEL" in selected_metric or selected_metric == "ECE":
        num_iter = 1000
    else:
        num_iter = 1000
    ...
    if "SEL" in selected_metric or selected_metric == "NLL":
        metric_values_list, test_metric_value = np.array(metric_values_list), np.array(test_metric_value)
...
def compute_metric_and_ci(y_true, predictions, selected_metric="AUROC"):
        ...
        if "SEL" in selected_metric or selected_metric == "NLL":
            ...
            (metric_per_label, upper_metric_value, lower_metric_value) = get_model_performance(df, selected_metric)
            metric_value = metric_per_label.mean()

            if "SEL" in selected_metric:
                ci_metric = list(zip(lower_metric_value, upper_metric_value)) # This is required because we have 25 CIs
            else:
            ...
            for i in range(y_true.shape[1]):
                ...
                (test_metric_value, upper_metric_value, lower_metric_value) = get_model_performance(df, selected_metric)
                metric_scores.append(test_metric_value)
                ci_metric.append((lower_metric_value, upper_metric_value))

            metric_per_label = np.array(metric_scores)
            metric_value = metric_per_label.mean()
            ci_metric = np.array(ci_metric)

        return metric_value, metric_per_label, ci_metric