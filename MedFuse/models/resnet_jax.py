...
class ResNet(nn.Module):
    ...
    def setup(self):
        ...
        if self.pretrained == 'imagenet':
            # This creates an error when you send a job array in SLURM
            # Download the file beforehand and add it to your working folder.
            # ckpt_file = utils.download(self.ckpt_dir, URLS[self.architecture]) 
            ckpt_file = 'resnet34_weights.h5'
    ...
    def __call__(self, x, train=True, feature=False):
        ...
        # In order to create the MedFuse Fusion model
        # The value is the same as the final ResNet34 layer
        # self.feats_dim = 512 # Apparently I cant do this in JAX. Variables are immutable
        # --------------------------------------------
